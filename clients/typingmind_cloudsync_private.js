  ...
  class ConfigManager {
    ...
    loadConfig() {
      ...
      const keyMap = {
        ...
        googleUserEmail: "tcs_google_useremail",
      };
    ...
    loadExclusions() {
      ...
      const systemExclusions = [
        ...
        "tcs_aws_endpoint",
        "tcs_google_clientid",
        "tcs_google_serverurl",
        "tcs_google_connectiontoken",
        "tcs_google_useremail",
...
async download(key, isMetadata = false) {
  ...
  class GoogleDriveService extends IStorageProvider {
    ...
    static get displayName() {
      return "Google Drive (Private Server OAuth)";
    }
    ...
    static getConfigurationUI() {
      const html = `
        <div class="space-y-2">
          <div>
            <label for="google-server-url" class="block text-sm font-medium text-zinc-300">Private Sync Server URL <span class="text-red-400">*</span></label>
            <input id="google-server-url" name="google-server-url" type="text" class="w-full px-2 py-1.5 border border-zinc-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700 text-white" placeholder="https://your-secret-subdomain.example.com" autocomplete="off" required>
          </div>
          <div>
            <label for="google-connection-token" class="block text-sm font-medium text-zinc-300">Google Connection Token</label>
            <input id="google-connection-token" name="google-connection-token" type="password" class="w-full px-2 py-1.5 border border-zinc-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm dark:bg-zinc-700 text-white" autocomplete="off">
          </div>
          <div class="pt-1 space-y-2">
            <button id="google-auth-btn" class="w-full inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-gray-500 disabled:cursor-default transition-colors">Authenticate via Private Server</button>
            <button id="google-revoke-btn" class="w-full inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-zinc-600 hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-500 disabled:bg-gray-500 disabled:cursor-default transition-colors">Revoke Private Connection</button>
            <div id="google-auth-status" class="text-xs text-center text-zinc-400 pt-1"></div>
          </div>
          <div class="text-xs text-zinc-400 rounded-md border border-zinc-700 p-2 bg-zinc-900">
            Google OAuth tokens remain only on your private server. This extension stores only a private connection token and keeps TypingMind out of the Google token flow.
          </div>
        </div>
      `;

      const setupEventListeners = (container, providerInstance, config, logger) => {
        const serverUrlInput = container.querySelector("#google-server-url");
        const tokenInput = container.querySelector("#google-connection-token");
        const authBtn = container.querySelector("#google-auth-btn");
        const revokeBtn = container.querySelector("#google-revoke-btn");
        const statusEl = container.querySelector("#google-auth-status");

        serverUrlInput.value = config.get("googleServerUrl") || "";
        tokenInput.value = config.get("googleConnectionToken") || "";

        const updateStatus = async () => {
          const currentServerUrl = serverUrlInput.value.trim().replace(/\/$/, "");
          const currentToken = tokenInput.value.trim();
          if (!currentServerUrl) {
            authBtn.disabled = true;
            revokeBtn.disabled = true;
            statusEl.textContent = "Status: Private server URL required.";
          ...
          if (!currentToken) {
            revokeBtn.disabled = true;
            statusEl.textContent = "Status: Not authenticated yet.";
            statusEl.style.color = "";
            return;
          }
          ...
          try {
            const resp = await fetch(`${currentServerUrl}/gdrive-sync/auth/status?app=sync`, {
              method: "GET",
              headers: { "X-Connection-Token": currentToken },
              cache: "no-store",
            });
            if (!resp.ok) throw new Error(`status ${resp.status}`);
            const data = await resp.json();
            const email = data?.connection?.userEmail || config.get("googleUserEmail") || "connected";
            statusEl.textContent = `Status: Connected as ${email}`;
            statusEl.style.color = "#22c55e";
          } catch (error) {
            statusEl.textContent = "Status: Connection token invalid or expired. Re-authenticate.";
        ...
        const handleAuth = async () => {
          const currentServerUrl = serverUrlInput.value.trim().replace(/\/$/, "");
          if (!currentServerUrl) {
            alert("Please enter your private sync server URL first.");
            return;
          }
          config.set("googleServerUrl", currentServerUrl);
          config.save();
          try {
            authBtn.disabled = true;
            authBtn.textContent = "Authenticating...";
            statusEl.textContent = "Waiting for Google sign-in through your private server...";
            const tempProvider = new GoogleDriveService(config, providerInstance.crypto, logger);
            await tempProvider.initialize();
            await tempProvider.handleAuthentication({ interactive: true });
            tokenInput.value = config.get("googleConnectionToken") || "";
            statusEl.textContent = `✅ Connected as ${config.get("googleUserEmail") || "authenticated account"}. Please click Save.`;
            statusEl.style.color = "#22c55e";
          } catch (error) {
            logger.log("error", "Private Google authentication failed", error);
            statusEl.textContent = `❌ Auth failed: ${error.message}`;
            statusEl.style.color = "#ef4444";
          } finally {
            authBtn.disabled = false;
            authBtn.textContent = "Authenticate via Private Server";
          }
        };
        ...
        const handleRevoke = async () => {
          const currentServerUrl = serverUrlInput.value.trim().replace(/\/$/, "");
          const currentToken = tokenInput.value.trim();
          if (!currentServerUrl || !currentToken) return;
          if (!confirm("Revoke the stored Google connection on your private server?")) return;
          try {
            revokeBtn.disabled = true;
            const resp = await fetch(`${currentServerUrl}/gdrive-sync/auth/revoke`, {
              method: "DELETE",
              headers: { "X-Connection-Token": currentToken },
              cache: "no-store",
            });
            if (!resp.ok) throw new Error(`status ${resp.status}`);
            config.set("googleConnectionToken", "");
            config.set("googleUserEmail", "");
            config.save();
            tokenInput.value = "";
            statusEl.textContent = "Connection revoked. Re-authenticate when ready.";
        ...
        serverUrlInput.addEventListener("input", updateStatus);
        tokenInput.addEventListener("input", () => {
          config.set("googleConnectionToken", tokenInput.value.trim());
          config.save();
          updateStatus();
        });
    ...
    isConfigured() {
      return !!(this.config.get("googleServerUrl") && this.config.get("googleConnectionToken"));
    }

    async initialize() {
      this.serverUrl = (this.config.get("googleServerUrl") || "").replace(/\/$/, "");
      this.connectionToken = this.config.get("googleConnectionToken") || "";
      this.userEmail = this.config.get("googleUserEmail") || "";
      if (!this.serverUrl) {
        throw new Error("Google private server URL is not configured");
      }
    }

    _storeConnection(connectionToken, userEmail) {
      if (!connectionToken) return;
      this.connectionToken = connectionToken;
      this.userEmail = userEmail || "";
      this.config.set("googleConnectionToken", connectionToken);
      this.config.set("googleUserEmail", userEmail || "");
      this.config.save();
    }
    ...
    async _openAuthPopup(url) {
      return new Promise((resolve, reject) => {
        ...
        const popup = window.open(
          url,
          "tm-private-google-auth",
          `width=${width},height=${height},left=${left},top=${top},popup=yes`
        );

        if (!popup) {
          reject(new Error("Popup blocked. Please allow popups for TypingMind and try again."));
          return;
        }
        ...
        const onMessage = (event) => {
          ...
          if (data.type !== "TM_PRIVATE_GOOGLE_AUTH_RESULT") return;
          cleanup();
          if (!data.ok) {
            reject(new Error(data.error || "Authentication failed."));
            return;
          }
          this._storeConnection(data.deviceToken, data.userEmail || "");
    ...
    async _request(path, options = {}) {
      ...
      if (!this.connectionToken) {
        throw new Error("Google connection token is missing. Authenticate first.");
      }
      const headers = new Headers(options.headers || {});
      headers.set("X-Connection-Token", this.connectionToken);
    ...
    async handleAuthentication(options = { interactive: false }) {
      if (this.isConfigured()) {
        try {
          ...
          this.logger.log("warning", `Stored private Google connection is invalid: ${error.message}`);
        }
      }
      ...
      const authUrl = `${this.serverUrl}/gdrive-sync/auth/start?app=sync&mode=popup&origin=${origin}`;
      await this._openAuthPopup(authUrl);
    }
    ...
    async upload(key, data, isMetadata = false, itemKey = null) {
      ...
      if (isMetadata) {
        ...
        body = await this.crypto.encryptBytes(data);
        contentType = "application/octet-stream";
      } else {
        body = await this.crypto.encrypt(data, itemKey || key);
        contentType = "application/octet-stream";
      }

      const response = await this._request(`/gdrive-sync/storage/object?key=${encodeURIComponent(key)}&metadata=${isMetadata ? "true" : "false"}`, {
        method: "PUT",
    ...
    async delete(key) {
      await this._request(`/gdrive-sync/storage/object?key=${encodeURIComponent(key)}`, { method: "DELETE" });
    }

    async deleteFolder(folderPath) {
      await this._request(`/gdrive-sync/storage/folder?path=${encodeURIComponent(folderPath)}`, { method: "DELETE" });
    }

    async list(prefix = "") {
      const response = await this._request(`/gdrive-sync/storage/list?prefix=${encodeURIComponent(prefix)}`, { method: "GET" });
      const data = await response.json();
      return data.items || [];
    }
    ...
    async copyObject(sourceKey, destinationKey) {
      await this._request(`/gdrive-sync/storage/copy`, {
        method: "POST",
    ...
    async verify() {
      const response = await this._request(`/gdrive-sync/auth/status?app=sync`, { method: "GET" });
      const data = await response.json();
      this.userEmail = data?.connection?.userEmail || this.userEmail;
      if (this.userEmail) {
        this.config.set("googleUserEmail", this.userEmail);
        this.config.save();
      }
    }

    async ensurePathExists(path) {
      await this._request(`/gdrive-sync/storage/ensure-path`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    async detectChanges() {
      ...
      for await (const batch of itemsIterator) {
        for (const item of batch) {
          ...
          if (this.config?.shouldExclude?.(key)) continue;
      ...
      for (const [itemId, metadata] of Object.entries(this.metadata.items || {})) {
        if (metadata.deleted && metadata.deleted > (metadata.synced || 0)) {
          if (!changedItems.some((item) => item.id === itemId && item.reason === "tombstone")) {
            changedItems.push({
              id: itemId,
              type: metadata.type,
      ...
      try {
        ...
        const processOneUpload = async (item) => {
          ...
          if (cloudItem && !item.deleted) {
            const cloudDeletedTs = this.getComparableTimestamp(cloudItem.deleted || 0);
            const cloudModifiedTs = this.getComparableTimestamp(cloudItem.lastModified || 0);
            const localModifiedTs = this.getComparableTimestamp(item.lastModified || 0);
            if (cloudDeletedTs > localModifiedTs || cloudModifiedTs > localModifiedTs) {
              this.logger.log(
                "skip",
    ...
    async createInitialSync() {
      ...
      try {
        if (this.storageService instanceof GoogleDriveService) {
          await this.storageService.ensurePathExists("items");
        }
  ...
  class CloudSyncApp {
    ...
    async initialize() {
      ...
      this.leaderElection.onBecameLeader(() => {
        this.logger.log(
          "info",
          "👑 This tab is now the leader. Waiting briefly before background tasks for leadership stabilization."
        );
        clearTimeout(this._leaderStartTimeout);
        this._leaderStartTimeout = setTimeout(() => {
          this.runLeaderTasks();
        }, 3000);
      });
    ...
    checkMandatoryConfig() {
      ...
      if (storageType === "googleDrive") {
        return !!(
          ...
          this.config.get("googleConnectionToken") &&
          this.config.get("encryptionKey")
        );
      }
      return false;
    ...
    getConfigFromUrlParams() {
      ...
      const paramMap = {
        ...
        googleserverurl: "googleServerUrl",
        googleconnectiontoken: "googleConnectionToken",
      };
    ...
    populateFormFromUrlParams(modal) {
      ...
      const fieldMap = {
        ...
        googleServerUrl: "google-server-url",
        googleConnectionToken: "google-connection-token",
      };
    ...
    async saveSettings(modal) {
      ...
      if (storageType === "s3") {
        ...
        newConfig.googleServerUrl = providerContainer
          ...
          .value.trim();
        newConfig.googleConnectionToken = providerContainer
          .querySelector("#google-connection-token")
          .value.trim();
        newConfig.googleUserEmail = this.config.get("googleUserEmail") || "";
      }
