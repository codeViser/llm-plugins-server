(() => {
  /******************************************************************
   * TypingMind Exporter v9
   *
   * Desktop: fully automated (sidebar full-export → fallback share JSON)
   * Mobile/PWA: passive listener with dismissible overlay
   * Zero Blob/createObjectURL overrides
   * Brief HTMLAnchorElement.prototype.click override only during capture
   ******************************************************************/
  ...
  async function desktopTriggerShareJSON() {
    ...
    const btn = [...modal.querySelectorAll("button")].find((b) => textOf(b) === "JSON");
  ...
  function SH(s) {
    ...
    el.querySelectorAll("*").forEach((x) => { for (let i = x.attributes.length - 1; i >= 0; i--) { const a = x.attributes[i].name; if (/^on/i.test(a)) x.removeAttribute(a); if (a === "href" && (x.getAttribute(a) || "").trim().toLowerCase().startsWith("javascript:")) x.setAttribute(a, "#"); } });
    return el.innerHTML;
  }
  ...
  function buildMaps(ms) {
    const a = {}, n = {};
    for (let i = 0; i < ms.length; i++) { const m = ms[i]; if (m.role === "assistant" && Array.isArray(m.tool_calls)) { const bn = {}; m.tool_calls.forEach((tc) => { const fn = tc.function || tc, nm = fn.name || "", ag = fn.arguments || "{}"; if (tc.id) n[tc.id] = nm; try { bn[nm] = typeof ag === "string" ? JSON.parse(ag) : ag; } catch { bn[nm] = { raw: ag }; } }); a[i] = bn; } }
  ...
  function buildChatHTML(ms) {
    ...
    for (let i = 0; i < ms.length; i++) {
      ...
      while (j < ms.length && ms[j].role !== "user") {
        ...
        } else if (n.role === "assistant") { const ac = S(n.content); if (ac.trim().length > 2) replies.push(ac); }
        j++;
      }
      out += tools;
  ...
  /* ============ MOBILE FLOWS (listener + overlay) ============ */
  let mobileCapture = null; // active capture reference
  ...
  async function mobileFlow(mode) {
    ...
    // Create capture (accepts both full and small JSON; does NOT suppress download)
    ...
    bar.innerHTML = `
      ...
      <div style="margin-top:6px;font-size:12px;opacity:.8">
        Go to <b>sidebar → kebab (⋯) → Export</b> for full JSON.<br>
        Or use <b>Share → JSON</b> for basic export.
      </div>
      <div class="row"></div>
    `;
  ...
  async function doHTML() {
    if (isMobileLike()) return mobileFlow("html");
    return desktopFlow("html");
  }
  async function doPDF() {
    if (isMobileLike()) return mobileFlow("pdf");
    return desktopFlow("pdf");
  }

  /* ============ INJECT BUTTONS ============ */
  function inject(modal) {
    if (!modal || modal.querySelector("[data-tmx='v9']")) return;
    ...
    const addRow = (btn, desc) => {
      const l = document.createElement("div"); l.className = "flex items-center justify-end"; l.appendChild(btn);
      const r = document.createElement("div"); r.textContent = desc;
      grid.appendChild(l); grid.appendChild(r);
    };

    addRow(
      mkBtn("Interactive HTML", "#00a884", doHTML),
      mobile ? "Starts listener → you manually Export from sidebar." : "One-click: full export (fallback: share JSON)."
    );
    addRow(
      mkBtn("Nice PDF", "#f59e0b", doPDF),
      mobile ? "Starts listener → you manually Export → opens print." : "One-click: full export → print window."
    );

    const m = document.createElement("div"); m.setAttribute("data-tmx", "v9"); m.style.display = "none"; modal.appendChild(m);
    log("Buttons injected.", mobile ? "(mobile mode)" : "(desktop mode)");
  }
  ...
  log("v9 loaded.", isMobileLike() ? "Mobile mode." : "Desktop mode.");
})();
