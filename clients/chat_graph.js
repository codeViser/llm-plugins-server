...
(() => {
  ...
  function restoreViewState(panel) {
    ...
    const cs = getChatState();
    const cid = cs?.state?.chatID ?? null;
    if (sv.chatID && cid && sv.chatID !== cid) return false;
  ...
  function getChatScroller(preferredEl = null) {
    ...
    if (chatSpace) {
      const cands = [];
      if (isScrollable(chatSpace)) cands.push(chatSpace);
      const ns = chatSpace.querySelectorAll('[data-element-id],[class],[style],div,section');
      for (const n of ns) if (isScrollable(n)) cands.push(n);
      const best = pickBestScroller(cands);
      if (best) return best;
    }
  ...
  function findMessageBlock(tsBtn) {
    ...
    const sels = ['[data-element-id="response-block"]','[data-element-id="request-block"]','[data-element-id="message-block"]','[data-element-id*="message"]','[data-element-id*="block"]'];
    for (const sel of sels) { const e = tsBtn.closest(sel); if (e) return e; }
  ...
  function locateMessageBlock(uuid) {
    ...
    for (const sel of [`[data-message-id="${uuid}"]`,`[data-uuid="${uuid}"]`,`[data-message-uuid="${uuid}"]`,`[data-id="${uuid}"]`,`[id="${uuid}"]`]) {
      const b = normalizeBlock(document.querySelector(sel));
      if (b) return { block: b, method: 'attr-exact' };
    }
    for (const sel of [`[data-element-id*="${uuid}"]`,`[id*="${uuid}"]`]) {
      const b = normalizeBlock(document.querySelector(sel));
      if (b) return { block: b, method: 'attr-partial' };
    }
  ...
  async function openMinimapPanel() {
    ...
    if (!mb) {
      const ta = document.querySelector('[data-element-id="current-chat-title"]');
      if (ta) mb = ta.querySelector('button[aria-haspopup="menu"]');
    }
    if (!mb) return false;
    mb.click();
  ...
  function findMinimapButton(rawContent) {
    ...
    const btns = mm.querySelectorAll('button[type="button"]');
    if (!btns.length) return null;
    for (const sl of [38, 22, 12]) {
      const snip = target.slice(0, sl).toLowerCase();
      if (snip.length < 4) continue;
      for (const btn of btns) {
        const p = btn.querySelector('p');
        if (!p) continue;
        if (p.textContent.replace(/\s+/g, ' ').trim().toLowerCase().includes(snip)) return btn;
      }
    }
    return null;
  }
  ...
  function getMetaLabel(toolMsgs) {
    ...
    if (names.length > 0) {
      const p = names.slice(0, 3).join(', ');
      return names.length > 3 ? p + ` +${names.length - 3}` : p;
    }
    ...
    const parts = toolMsgs.map(m => {
      if (m.role === 'assistant' && m.tool_calls?.length > 0) {
        const calls = m.tool_calls.map(tc => {
          const nm = tc.function?.name || tc.name || 'tool';
          let args = '';
  ...
  function treeCols(n) {
    if (!n) return 1;
    if (n.variants.length > 0) return treeCols(n.children[0] || null) + n.variants.reduce((s, v) => s + treeCols(v), 0);
    return n.children.length ? treeCols(n.children[0]) : 1;
  }
  function placeNode(node, cx, cy, all, edges) {
    ...
    if (node.variants.length > 0) {
      ...
      if (node.children[0]) {
        ...
        edges.push({ fx: cx, fy: cy + NH, tx: aCx, ty: ny, active: true });
        placeNode(node.children[0], aCx, ny, all, edges); sx += ac * slotW;
      }
      node.variants.forEach((v, vi) => {
        const vCx = sx + colsPx(vcs[vi]) / 2;
        edges.push({ fx: cx, fy: cy + NH, tx: vCx, ty: ny, active: false });
        placeNode(v, vCx, ny, all, edges); sx += vcs[vi] * slotW;
      });
  ...
  function showToast(msg, type = 'ok') {
    if (!toastEl) return;
    clearTimeout(toastTmr); toastEl.textContent = msg; toastEl.className = type;
  ...
  function openPreview(node, panelEl) {
    ...
    const roleLabel = isMeta
      ? `Tool Calls (\u00d7${node.toolMessages?.length || 1})`
      : node.isSummary ? '\u2211 Summary'
      : node.role === 'user' ? 'USER' : 'AI Response';
    ...
    if (isMeta) {
      ...
      if (ia) {
        ...
        badgeClass = 'inactive';
        badgeText = `\u2211 ${steps > 1 ? steps + ' switches' : '1 switch'} to activate`;
      }
    } else {
      if (ia) {
    ...
    const mkNavBtn = (label, title) => {
      const b = document.createElement('button'); b.className = EXT + '-pbtn nav'; b.title = title; b.innerHTML = label; return b;
    };
  ...
  function computeSingleSwitch(msgs, srcU, bi) {
    const si = msgs.findIndex(m => m.uuid === srcU);
    if (si < 0) throw new Error(`UUID ${srcU} not found`);
    const sm = msgs[si], tgt = sm.threads?.[bi];
    if (!tgt) throw new Error(`threads[${bi}] missing`);
    return [
      ...msgs.slice(0, si),
      { ...sm, content: tgt.userMessageContent, threads: [...sm.threads.filter((_, i) => i !== bi), { userMessageContent: sm.content, messages: msgs.slice(si + 1), createdAt: new Date().toISOString() }], updatedAt: new Date().toISOString() },
      ...(tgt.messages || [])
    ];
  }
    ...
    window.addEventListener('keydown', e => {
      if (e.key === 'Escape') {
        ...
        if (nd?.active) {
          ...
          closePreview(panel); closeOverlay();
          if (target) scrollAfterClose(target, content);
        }
      }
    }, { signal: sig });
