...
export const handleTavilyCrawlProxy = async (req: Request, res: Response) => {
  ...
  if (!url) {
    const serviceResponse = new ServiceResponse(ResponseStatus.Failed, 'URL is missing in request body', null, StatusCodes.BAD_REQUEST);
    return handleServiceResponse(serviceResponse, res);
  }
  ...
  try {
    ...
    if (tavilyResponse.statusCode !== StatusCodes.OK) {
      ...
      const serviceResponse = new ServiceResponse(ResponseStatus.Failed, `Tavily Crawl API error: ${errorMessage}`, responseData, tavilyResponse.statusCode as StatusCodes);
      return handleServiceResponse(serviceResponse, res);
    }
