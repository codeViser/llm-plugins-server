...
googleWorkspaceRouter.get('/drive/files', verifyGoogleAccessTokenAndSetClient, async (req: Request, res: Response) => {
  try {
...
googleWorkspaceRouter.get('/drive/search', verifyGoogleAccessTokenAndSetClient, async (req: Request, res: Response) => {
  try {
...
googleWorkspaceRouter.get(
  ...
  verifyGoogleAccessTokenAndSetClient,
        ...
        try {
          ...
          try {
            ...
            const result = await mammoth.extractRawText({
              buffer: Buffer.from(fileContentResponse.data as ArrayBuffer),
            });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
        ...
        details: error.message,
        recommendation:
          'For Google Workspace files, consider using drive_read_file_content for text extraction instead',
      });
    }
  }
);
...
googleWorkspaceRouter.post('/drive/files', verifyGoogleAccessTokenAndSetClient, async (req: Request, res: Response) => {
  try {
