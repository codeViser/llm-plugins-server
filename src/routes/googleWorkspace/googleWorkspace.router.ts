...
googleWorkspaceRouter.get('/drive/files', async (req: Request, res: Response) => {
  ...
  try {
    const listParams: any = { // Type as any for flexibility with query params
        pageSize: 20,
        fields: 'nextPageToken, files(id, name, mimeType, webViewLink, iconLink, modifiedTime, createdTime, owners, shared, capabilities)',
    ...
    if (error.response && error.response.status === 401) {
        return res.status(StatusCodes.UNAUTHORIZED).json({ error: 'Google API authentication error. Please re-authenticate.', details: error.message });
    }
...
googleWorkspaceRouter.get('/drive/files/:fileId/content', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (mimeType === 'application/vnd.google-apps.document') {
            ...
            res.status(StatusCodes.OK).json({ id: fileId, name: metadataResponse.data.name, mimeType, content, fullDoc: docResponse.data });
            ...
            res.status(StatusCodes.OK).json({ id: fileId, name: metadataResponse.data.name, mimeType, content: fileContentResponse.data });
        } else {
        ...
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: `Failed to read file ${fileId}`, details: error.message });
    }
});
...
googleWorkspaceRouter.post('/drive/files/:fileId/content', async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: `Failed to write to file ${fileId}`, details: error.message });
    }
});
...
googleWorkspaceRouter.post('/drive/files', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (mimeType === 'application/vnd.google-apps.document') {
            ...
            const file = await drive.files.create({ resource: fileMetadata, media: media, fields: 'id, name, mimeType, webViewLink' });
